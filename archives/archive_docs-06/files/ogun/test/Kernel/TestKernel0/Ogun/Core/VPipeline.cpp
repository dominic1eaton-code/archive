...
VGraphicsPipeline& VGraphicsPipeline::format(VkFormat format)
{
    m_swapchainImageFormat = format;
    return *this;
}

VGraphicsPipeline& VGraphicsPipeline::depth(VkFormat depth)
{
    m_depthFormat = depth;   
    return *this;
}
...
VGraphicsPipeline& VGraphicsPipeline::build()
{
    ...
    m_createInfo.pTessellationState = nullptr;              // Optional
...
void VGraphicsPipeline::populateRenderPasses()
{
...
void VGraphicsPipeline::populateShaderStages()
{
    ...
    for (int i=0; i<m_shaders.size(); i++)
    {
        ...
        shaderStageInfo.pName = m_shaders[i].entryPoint().c_str();
