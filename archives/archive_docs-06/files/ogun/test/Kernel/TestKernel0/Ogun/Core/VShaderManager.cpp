...
VShaderManager::VShaderManager(std::string rootPath)
    : m_root(rootPath)
    , m_shaders({})
    , compiler("C:/global/VulkanSDK/1.3.296.0/Bin/glslc.exe")
{}
...
void VShaderManager::load(VkDevice device)
{
    ...
    shaderFiles.push_back(std::filesystem::path(m_root) / "test" / "test0.vert.spv");
    shaderFiles.push_back(std::filesystem::path(m_root)  / "test" / "test0.frag.spv");
