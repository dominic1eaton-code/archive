...
uint16_t VulkanModel::switchActiveObject(uint8_t direction)
{
    uint32_t numObjectsInScene = m_scene->meshbuffer()->models().size();
    m_currentActiveObjectIndex = (m_currentActiveObjectIndex + direction) % (numObjectsInScene-1);
    return m_currentActiveObjectIndex;
}
