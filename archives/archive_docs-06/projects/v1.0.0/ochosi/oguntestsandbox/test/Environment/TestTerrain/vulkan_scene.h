...
public:
    ...
    void updateVertex(TVertex vertex, uint32_t index)
    {
...
public:
    ...
    void load(std::vector<TVertex>& vertices, std::vector<uint16_t>& indices, std::vector<GPUMeshData>& models, std::vector<GPUDirLight>& dirlights,
              glm::vec2 terrainHeightMapExtent);

    MeshBuffer* meshbuffer() const { return m_meshbuffer; }

    GPUCamera primaryCamera() const { return m_primaryCamera; }
