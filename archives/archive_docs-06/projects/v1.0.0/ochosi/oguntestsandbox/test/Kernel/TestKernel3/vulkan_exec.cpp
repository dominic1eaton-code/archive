...
void VulkanModel::init(VulkanModelInfo info)
{
    ...
    /* resources */
    // render to texture image, image texture
    std::vector<uint32_t> sceneTexturePixels; // default scene texture image of all 1 values
    uint32_t sceneTextureWidth = extent.width;
    uint32_t sceneTextureHeight = extent.height;
    uint32_t sceneTextureSize = sceneTextureHeight * sceneTextureHeight * 4;
    sceneTexturePixels.resize(sceneTextureSize);
    for (uint32_t i = 0; i < sceneTextureSize; i++)
    {
        sceneTexturePixels[i] = 1u;
    }
    VkImageView sceneTextureImageView;
    VkImage sceneTextureImage;
    VkSampler sceneTextureSampler;
    VkDeviceMemory sceneTextureImageMemory;
    // VulkanModelSupport::createTextureImage2(pdevice.primary(), m_device, sceneTextureWidth,sceneTextureHeight, sceneTexturePixels, ldevice.queues()[0], commandPool, pdevice.info().memoryProperties, sceneTextureImage, sceneTextureImageMemory, mipLevels);
    // VulkanModelSupport::createTextureImageView(m_device, sceneTextureImage, mipLevels, sceneTextureImageView);
    // VulkanModelSupport::createTextureSampler(pdevice.info().properties, m_device, sceneTextureSampler);
    ...
    // primitive ID output image
    VkFormat pformat = m_swapchain.format(); //VK_FORMAT_R8_UINT;
    VulkanModelSupport::createImage(m_device, m_swapchain.extent().width, m_swapchain.extent().height, 1, VK_SAMPLE_COUNT_8_BIT, pformat, VK_IMAGE_TILING_OPTIMAL,
    VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
    m_primitiveIDimage.image, m_primitiveIDimage.memory, pdevice.info().memoryProperties);
    m_primitiveIDimage.view = VulkanModelSupport::createImageView(m_device,  m_primitiveIDimage.image, pformat, VK_IMAGE_ASPECT_COLOR_BIT, 1u);
    ...
    VulkanModelSupport::createFramebuffers(m_device, m_swapchain.extent(), m_renderpass, m_primitiveIDimage.view , m_colorImageView, m_depthImageView, m_swapchain.imageViews(), m_framebuffers);
    m_queue = ldevice.queues()[0];

    // offscreen render
    offscreenpass.oColorImage.extent = m_swapchain.extent();
    offscreenpass.oColorImage.miplevels = 1u;
    offscreenpass.oColorImage.samples = VK_SAMPLE_COUNT_1_BIT;
    offscreenpass.oColorImage.format = m_swapchain.format();
    offscreenpass.oPrimitiveIDImage.extent = m_swapchain.extent();
    offscreenpass.oPrimitiveIDImage.miplevels = 1u;
    offscreenpass.oPrimitiveIDImage.samples = VK_SAMPLE_COUNT_1_BIT;
    offscreenpass.oPrimitiveIDImage.format = m_swapchain.format();
    offscreenpass.bpoint = bindPoint;
    offscreenpass.extent = m_swapchain.extent();
    offscreenpass.set_size = ogun::constants::MAX_FRAMES_IN_FLIGHT;
    VulkanModelSupport::createOffscreenPassObject(offscreenpass, m_device, pdevice, m_swapchain.format(), m_memProperties);
    ...
    m_scene->load(vertices, indices, meshes, dirlights);
    ...
    // VGraphicsPipeline spass0Pipeline;
    // spass0Pipeline.device(m_device)
    //          .pass(m_renderpass)
    ...
    //          .build();
    VGraphicsPipeline opipeline;
    opipeline.device(m_device)
             .pass(offscreenpass.rpass.pass())
             .extent(swapchain.extent())
             .layout(m_descriptorSetLayout)
             .shaders(m_shader->shaders())
             .samples(VK_SAMPLE_COUNT_1_BIT)
             .build();
    offscreenpass.gpipeline = opipeline;
    ...
    // m_pipelines.push_back(spass0Pipeline.pipeline());
...
void VulkanModel::draw(float tick)
{
    ...
    m_shader->update(tick, m_frameIndex, view, m_cursorPosition);
    ...
    VulkanModelSupport::recordCommandBuffer(m_commandBuffers[m_frameIndex], imageIndex, m_renderpass, extent, m_framebuffers[m_frameIndex], m_pipelines[0], 
                        ...
                        m_device, m_vertices, m_indices, m_queues[0], m_commandPool, m_memProperties, offscreenpass);
    ...
    // // get scene texture image
    // m_sceneTextureImage.image = offscreenpass.oColorImage.image;
    // m_sceneTextureImage.memory = offscreenpass.oColorImage.memory;
    // m_sceneTextureImage.extent = offscreenpass.oColorImage.extent;
    // m_sceneTextureImage.miplevels = offscreenpass.oColorImage.miplevels;
    std::vector<char> cdata;
    std::vector<char> pdata;
    VulkanModelSupport::retrieveTextureImage(offscreenpass.oColorImage, m_pdevice, m_device, m_queue, m_commandPool, m_memProperties, offscreenpass.oColorImage.miplevels, extent, cdata);
    VulkanModelSupport::retrieveTextureImage(offscreenpass.oPrimitiveIDImage, m_pdevice, m_device, m_queue, m_commandPool, m_memProperties, offscreenpass.oPrimitiveIDImage.miplevels, extent, pdata);
...
void VulkanModel::recompileShaders()
{
    ...
    std::filesystem::path compilerPath = mount / "/global/Vulkan/1.3.296.0/Bin";
    std::string compilerExe = "glslc.exe";
    std::filesystem::path compiler = mount / "/global/Vulkan/1.3.296.0/Bin/" / compilerExe;
    ...
    std::string shaderType = "primitive";
    std::string shader = "test_primitive";
    ...
    shaderFiles.push_back(std::filesystem::path(shaderLibraryPath) / shaderType / "test_primitive.vert.spv");
    // shaderFiles.push_back(std::filesystem::path(shaderLibraryPath) / shaderType / "test_primitive.geom.spv");
    shaderFiles.push_back(std::filesystem::path(shaderLibraryPath) / shaderType / "test_primitive.frag.spv");
    // shaderFiles.push_back(std::filesystem::path(shaderLibraryPath) / shaderType / "test_primitive.comp.spv");
...
void VulkanModel::rebuildSwapchain()
{
    ...
    cleanupSwapchain();
    createSwapchain();
    VulkanModelSupport::createColorResources(m_device, m_swapchain.format(), m_deviceinfo.msaaSamples, m_colorImage, m_colorImageMemory, m_swapchain.extent(), m_deviceinfo.memoryProperties, miplevels, m_colorImageView);
    VulkanModelSupport::createDepthResources(m_device, m_pdevice, m_deviceinfo.msaaSamples, m_depthImage, m_depthImageMemory, m_swapchain.extent(), m_deviceinfo.memoryProperties, miplevels, m_depthImageView);

    VkFormat pformat = m_swapchain.format(); // VK_FORMAT_R8_UINT
    VulkanModelSupport::createImage(m_device, m_swapchain.extent().width, m_swapchain.extent().height, 1, VK_SAMPLE_COUNT_1_BIT, pformat, VK_IMAGE_TILING_OPTIMAL,
    VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
    m_primitiveIDimage.image, m_primitiveIDimage.memory, m_deviceinfo.memoryProperties);
    m_primitiveIDimage.view = VulkanModelSupport::createImageView(m_device,  m_primitiveIDimage.image, pformat, VK_IMAGE_ASPECT_COLOR_BIT, miplevels);
