...
struct VTImage
{
    ...
    uint32_t miplevels; // make uint8_t
...
public:
    ...
    static void createOffscreenPassObject(OffscreenPass& opass, VkDevice device, VPhysicalDevice pdevice, VkFormat format, VkPhysicalDeviceMemoryProperties memProperties);
    ...
    static void createFramebuffers(VkDevice device, VkExtent2D extent, VkRenderPass renderpass, VkImageView primitiveIDView, VkImageView colorImageView, VkImageView depthImageView, std::vector<VkImageView> imageViews, std::vector<VkFramebuffer>& frameBuffers);
    ...
    static void copyImageToBuffer(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, VkQueue& queue, VkCommandPool& pool, VkDevice device);
    ...
    static void retrieveTextureImage(VTImage& timage, VkPhysicalDevice pdevice, VkDevice device, VkQueue& queue, VkCommandPool& pool,
        VkPhysicalDeviceMemoryProperties memProperties, uint32_t& mipLevels, VkExtent2D extent, std::vector<char>& idata);

    static void createTextureImage(VkPhysicalDevice pdevice, VkDevice device, std::string texture, VkQueue& queue, VkCommandPool& pool,
    ...
    static void createTextureImage2(VkPhysicalDevice pdevice, VkDevice device, uint32_t texWidth, uint32_t texHeight, std::vector<uint32_t> pixels, VkQueue& queue, VkCommandPool& pool, 
        VkPhysicalDeviceMemoryProperties memProperties, VkImage& textureImage, VkDeviceMemory& textureImageMemory, uint32_t& mipLevels);
};
