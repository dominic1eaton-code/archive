            ...
            std::string shaderPath)
{
    ...
    shaderFiles.push_back(std::filesystem::path(shaderPath) / "primitive" / "test_primitive.vert.spv");
    shaderFiles.push_back(std::filesystem::path(shaderPath) / "primitive" / "test_primitive.frag.spv");
    ...
    // object selection storage buffer set
    m_gpuSelectorBuffers.resize(ogun::constants::MAX_FRAMES_IN_FLIGHT);
    for (size_t i = 0; i < ogun::constants::MAX_FRAMES_IN_FLIGHT; i++)
    {
        VulkanModelSupport::createBuffer(m_device, sizeof(GPUSelector) * ogun::constants::DEPTH_ARRAY_SCALE, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, m_gpuSelectorBuffers[i].buffer, m_gpuSelectorBuffers[i].memory, m_memProperties);
    }
...
void TestShader0::update(float tick, uint32_t currentImage, glm::mat4 view, glm::vec2 gcursor)
{
    ...
    // std::cout << " gcursor position x y: " << gcursor.x << " : " << gcursor.y << std::endl;
    cursor.position = glm::vec4(gcursor.x, gcursor.y, 1.0f, 1.0f);
    memcpy(m_cursorViewBuffers[currentImage].data, &cursor, sizeof(cursor));

    // object selector
    GPUSelector selector;
    void* data;
    vkMapMemory(m_device, m_gpuSelectorBuffers[currentImage].memory, 0, sizeof(GPUSelector) * ogun::constants::DEPTH_ARRAY_SCALE, 0, &data);
        ...
        auto u = static_cast<float*>(data);
        // uint32_t* u = new uint32_t();
        // memcpy(u, data, (size_t)( sizeof(GPUSelector) * ogun::constants::DEPTH_ARRAY_SCALE));
        float SELECTED_OBJECT_ID = 0;
        size_t index = 0;
        for(size_t i=0;i<ogun::constants::DEPTH_ARRAY_SCALE*sizeof(float)*6;i+=sizeof(float))
        {
            if( u[i] != 0)
            {
        ...
        std::cout << " SELECTED_OBJECT_ID : " << SELECTED_OBJECT_ID <<  " at index : " << index / sizeof(float) << std::endl;
    vkUnmapMemory(m_device, m_gpuSelectorBuffers[currentImage].memory);
    
    // we have to zero out the memory each frame
    std::memset(data, 0, ogun::constants::DEPTH_ARRAY_SCALE * sizeof(float));
}
...
void TestShader0::createDescriptorSetLayout()
{
    ...
    VkDescriptorSetLayoutBinding selectorBinding{};
    selectorBinding.binding = 5;
    selectorBinding.descriptorCount = 1;
    selectorBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    selectorBinding.pImmutableSamplers = nullptr;
    selectorBinding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    ...
    uboLayoutBindings.push_back(selectorBinding);
                            ...
                            uint32_t numMeshes, uint32_t numDirLights)
{
    ...
    for (size_t i = 0; i < m_buffersetSize; i++) 
    {
        ...
        cursorUniformBufferInfo.range = sizeof(GPUCursor);
        ...
        VkDescriptorBufferInfo selectorSSBOInfo{};
        selectorSSBOInfo.buffer = m_gpuSelectorBuffers[i].buffer;
        selectorSSBOInfo.offset = 0;
        selectorSSBOInfo.range = sizeof(GPUSelector) * ogun::constants::DEPTH_ARRAY_SCALE;
        ...
        descriptorWrites[5].pBufferInfo = &selectorSSBOInfo;
