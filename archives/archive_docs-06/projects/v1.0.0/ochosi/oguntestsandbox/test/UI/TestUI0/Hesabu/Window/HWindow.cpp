...
LRESULT HWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        std::cout << "screen cursor position " << "x: " << cursorPosition.x << " : y: "<< cursorPosition.y << std::endl;
        std::cout << "window cursor position " << "x: " << cposx << " : y: "<< cposy << std::endl;
        std::cout << "NDC cursor position " << "x: " << cndcx << " : y: "<< cndcy << std::endl;

        ScreenToClient(m_hwnd, &cursorPosition);
        float ndcX = 2*(static_cast<float>(cursorPosition.x) / (windowRect.right - windowRect.left)) - 1;
        float ndcY = 2*(static_cast<float>(cursorPosition.y) / (windowRect.bottom - windowRect.top)) - 1;
        std::cout << "NDC sscreen to client cursor position " << "x: " << ndcX << " : y: "<< ndcY << std::endl;
