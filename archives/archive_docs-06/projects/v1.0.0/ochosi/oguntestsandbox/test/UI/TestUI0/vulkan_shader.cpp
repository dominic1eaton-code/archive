            ...
            std::string shaderPath)
{
    ...
    shaderFiles.push_back(std::filesystem::path(shaderPath) / "ui" / "default_ui.vert.spv");
    // shaderFiles.push_back(std::filesystem::path(shaderPath) / "ui" / "default_ui.geom.spv");
    shaderFiles.push_back(std::filesystem::path(shaderPath) / "ui" / "default_ui.frag.spv");
    shaderFiles.push_back(std::filesystem::path(shaderPath) / "ui" / "default_ui.comp.spv");
...
void TestShader0::update(float tick, uint32_t currentImage, glm::mat4 view, glm::vec2 gcursor)
{
    ...
    camera.view = view;
    // camera.proj = glm::ortho(-(800.0f / 2.0f), 800.0f / 2.0f, 600.0f / 2.0f, -(600.0f / 2.0f), -1000.0f, 1000.0f); 
    // camera.proj = glm::ortho(-1.0f, 2.0f, -0.1f, 1.0f, 0.1f, 1.0f);
    // camera.proj = glm::ortho(0.0f, 800.0f, 0.0f, 600.0f, 0.1f, 100.0f);
    // camera.proj = glm::ortho(8.0f, 6.0f, 0.0f, 0.0f, 0.1f, 1.0f);
    ...
    cursor.position = glm::vec2(2.0f, 0.0f);
