...
VBuffer::VBuffer(VkDevice device, VkPhysicalDeviceMemoryProperties memoryProperties, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties)
    : m_device(device)
    , m_sType(VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO)
    , m_pNext(NULL)
    , m_flags(0)
...
VBuffer::VBuffer(VkDevice device, VkPhysicalDeviceMemoryProperties memoryProperties, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties)
    : m_device(device)
    , m_sType(VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO)
    , m_pNext(NULL)
    , m_flags(0)
...
void VBuffer::build()
{
    ...
    createvk(vkCreateBuffer(m_device,
                            &m_bufferInfo,
                            nullptr,
                            &m_buffer));
...
VkDeviceSize VBuffer::size()
{
    return m_size;
}

void VBuffer::allocate(VkDeviceSize size)
{
    m_size = size;
    build();
}

void VBuffer::deallocate()
{
