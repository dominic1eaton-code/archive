...
void VScene::load(std::vector<VertexShaderData>& vertices, std::vector<uint16_t>& indices, std::vector<GPUMeshData>& models, std::vector<GPUDirLight>& dirlights)
{
    ...
    Square* sq0 = new Square(glm::vec3(0.5f, .0f, 0.0f), glm::vec3(0.2, 0.8, 0.2), 0.1f, glm::vec4{1.0, 1.0, 0.0, 1.f});
    Square* sq1 = new Square(glm::vec3(0.f, 1.f, 0.0f), glm::vec3(5.0, 0.2, 1.0), 0.2f, glm::vec4{1.0, 0.25, .1, 1.f});
    Square* sq2 = new Square(glm::vec3(-.5f, -.5f, 0.0f), glm::vec3(1.0f, 1.5f, 1.0), 0.2f, glm::vec4{1.0, 0.0, 0.0, 0.5f});
    m_meshbuffer->append(sq0);
    m_meshbuffer->append(sq1);
    m_meshbuffer->append(sq2);
    ...
    models = meshes;
    vertices = m_meshbuffer->vertices();
    indices = m_meshbuffer->indices();
    // vertices = verts;
    // indices = inds;
}