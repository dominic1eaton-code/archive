...
{
    std::string shaderLibraryPath = std::filesystem::path(std::filesystem::path(info.mount) / "/dev/v0.0.1/ogun/assets/shaders").string();
    std::string testTextureImage = std::filesystem::path(std::filesystem::path(info.mount) / "/dev/v0.0.1/ogun/assets/textures/grass-texture-background.jpg").string();
    ...
    // std::string testTextureImage = "C:/dev/projects/ogun/assets/textures/grass-texture-background.jpg";
    uint32_t mipLevels;
    ...
    load_scene(vertices, indices, meshes, dirlights);
    ...
    m_shader->load(vertices, indices, meshes, dirlights,
                   m_queues[0], commandPool, pdevice.info().memoryProperties, m_device, m_swapchain.extent(),
                   descriptorSetLayout, textureSampler, textureImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    // m_indexBuffer = m_shader->indexBuffer();
    // m_vertexBuffers = m_shader->vertexBuffers();

    VShaderManager* shaderManager = new VShaderManager(shaderLibraryPath);
...
void VOgunExecutor::configure()
{
    if (!m_bWindowCreated)
    {
        ...
        if (!(m_bVulkanInitialized && m_bWindowCreated))
        {
            ...
            info.mount = "D:";
            // info.shaderLibraryPath = "C:/dev/projects/ogun/assets/shaders";
            // info.shaderLibraryPath = "C:/dev/projects/v0.0.1/ogunv2/ogun/assets/shaders";
            m_model->init(info);
        }
    }
}
