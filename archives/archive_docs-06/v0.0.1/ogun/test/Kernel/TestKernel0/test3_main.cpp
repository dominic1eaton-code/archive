...
void VulkanModel::init(VulkanModelInfo info)
{
    ...
    std::string testTextureImage = "C:/dev/projects_v1.0.0/ogun/assets/textures/grass-texture-background.jpg";
    uint32_t mipLevels;
...
void VOgunExecutor::configure()
{
    if (!m_bWindowCreated)
    {
        ...
        if (!(m_bVulkanInitialized && m_bWindowCreated))
        {
            ...
            info.shaderLibraryPath = "C:/dev/projects_v1.0.0/ogun/assets/shaders";
            m_model->init(info);
        }
    }
}
