...
public:
    ...
    void update(float tick, uint32_t currentImage)
    {
        ...
        camera.model = glm::rotate(glm::mat4(1.0f), tick * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        camera.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        camera.proj = glm::perspective(glm::radians(45.0f), m_extent.width / (float) m_extent.height, 0.1f, 10.0f);
        camera.proj[1][1] *= -1;
        memcpy(m_gpuCameraBuffers[currentImage].data, &camera, sizeof(camera));

        // GPUView view;
        // view.position = glm::mat4(1.0f);
        // memcpy(m_gpuViewBuffers[currentImage].data, &view, sizeof(view));
    }
...
private:
    ...
    uint32_t m_buffersetSize = ogun::constants::MAX_FRAMES_IN_FLIGHT;
    ...
    void createVertexBuffer(std::vector<VertexShaderData> vertices, VBuffer& vbuffer, VkDeviceSize buffersize)
    {
        ...
        createBuffer(m_device, buffersize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory, m_memProperties);
        void* data;
        vkMapMemory(m_device, stagingBufferMemory, 0, buffersize, 0, &data);
            memcpy(data, vertices.data(), (size_t) buffersize);
        vkUnmapMemory(m_device, stagingBufferMemory);
        createBuffer(m_device, buffersize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vbuffer.buffer, vbuffer.memory, m_memProperties);
        copyBuffer(stagingBuffer, vbuffer.buffer, buffersize, m_queue, m_device, m_pool);
    ...
    void createIndexBuffer(std::vector<uint16_t> indices, VBuffer& vbuffer, VkDeviceSize buffersize) 
    {
        ...
        vkUnmapMemory(m_device, stagingBufferMemory);
        createBuffer(m_device, buffersize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vbuffer.buffer, vbuffer.memory, m_memProperties);
        copyBuffer(stagingBuffer, vbuffer.buffer, buffersize, m_queue, m_device, m_pool);
        vkDestroyBuffer(m_device, stagingBuffer, nullptr);
        vkFreeMemory(m_device, stagingBufferMemory, nullptr);
    }
    ...
    void createUniformBuffers(std::vector<VBuffer>& vbuffer, VkDeviceSize buffersize) 
    {
        ...
        {
            createBuffer(m_device, buffersize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vbuffer[i].buffer, vbuffer[i].memory, m_memProperties);
    ...
    void createStorageBuffers(std::vector<T> vdata, std::vector<VBuffer>& vbuffer, VkDeviceSize buffersize)
    {
        ...
        for (size_t i = 0; i < m_buffersetSize; i++) {
            createBuffer(m_device, buffersize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vbuffer[i].buffer, vbuffer[i].memory, m_memProperties);
            copyBuffer(stagingBuffer, vbuffer[i].buffer, buffersize, m_queue, m_device, m_pool);
        }
        vkDestroyBuffer(m_device, stagingBuffer, nullptr);
        vkFreeMemory(m_device, stagingBufferMemory, nullptr);
    }
