...
const std = @import("std");
const gen = @import("gen/appmgmt.application_service_server.zig");
const Db = @import("db.zig").Db;
const KafkaProducer = @import("kafka_producer.zig").KafkaProducer;
...
pub const ApplicationService = struct {
    ...
    pub fn CreateApplication(
        ...
        req: *gen.CreateApplicationRequest,
    ) !gen.ApplicationResponse {
        const id = try self.db.createApp(req.name, req.version);
    ...
    pub fn GetApplication(
        self: *ApplicationService,
        req: *gen.GetApplicationRequest,
    ) !gen.ApplicationResponse {
        const row_opt = try self.db.getApp(req.id);
        if (row_opt == null) return error.NotFound;
    ...
    pub fn ListApplications(
        self: *ApplicationService,
        _: *gen.ListApplicationsRequest,
    ) !gen.ListApplicationsResponse {
        var rows = try self.db.listApps();
        defer rows.deinit();

        var array = std.ArrayList(gen.Application).init(std.heap.page_allocator);
        ...
        return gen.ListApplicationsResponse{
            .apps = array.toOwnedSlice(),
        };
    }
};
