...
what is a dimension? what are dimensions?
...
* event, change, transform, preservation, invariance, interactivity, subset(s)/"sliding-window"/bandwidth
* movement, vibration, vibrons, stasisons, energy, work, manifold, field, qauntization, (symbolic) representation, observability, memorability, associativity, commutativity, closure




## formalize+define+derive
* preservation
* invariance
* interaction preservation+invariance
* resources/resource
* singularity+duality+plurality
    - the|a singularity
    - the|a duality
    - the|a plurality
* associativity, commutativity, closure

- preservation -> invariance through/across change+transform. maintainence, homeostasis+allostasis

- the "sliding context window" of intelligent (and reasoning (subsystem of intelligence)) systems
    - intelligent systems/agents have boundary constrained+limitied context windows for accessing information, processing data, and interacting with other systems+agents


* mungu-corp vs mungu-inc
* imperialism, empire, colonization, colony, colonized, colonial, imperial, kingdom
* reality, realm, domain
* transfer, transference
* emergence, de-emergence, dissolution, insolvent, insolvency rebirth, renewal
* energy cost|price|incur(rence)|expense
* simulation, model
* solution, platform, application, product, service, promgram, portfolio, process, cycle, loop, string, system
* bandwidth, bandwidth theory B-theory ~ sliding+limited+constrained context window ~ filter+bias theory ~ compressions+clsoure+survival+entropy+complxity pressure
* organization systems, business systems, production systems, manufacturing sytems, fabrication systems, entity|product|service|solution|platform|application systems, cell(ular) systems, grammar systems, system of sytsems, automata systems, terraformata sytsms, entity+actor+agent systems, software|digital|twin system(s), digital twin systems ~ cyberphysical systems, mirror systems, art systems, music systems
* stable|unstable grammars, convergence|divergence, converge on|to(ward) stable grammars

* books: the business vehicle (series) ~ the business strategy, the business plan, the business tactic, the business operation, the business entity, the business solution, the business cycle
 
* cycle theory C-theory ~ the theory of cycle(s) anf continuous persistent invariamt survival
* simulation|model theory; the theory of syste(s)
* cycle(s), the agent+entity+actor cycle, the system cycle, grammar cycles, structure+flow cycles, maturity cycle, CMMI cycle ~ !capability maturity model index/cycle, capability cycle, twin cycles, singularity|duality|plurality cycles, generativity-structurativity cycles, L-cycles (of L-systems, L-grammars, L-generators, etc...), compression-decompression cycles

* solveability, problem solveability, solveability defintion/formalization, solution, problem

## associations
- memory + preservation + invariance
...
- why were poitners created (in programming) ? what was the context of thier emergence?
...
- DNA|cell information, copy, replication, every cell in an organism has a copy of a core DNA code?
...
Force, field, and fluid are foundational, interconnected concepts in physics that describe how, where, and through what medium physical interactions occur.

Force is the actual push or pull experienced by an object (e.g., gravity, friction).

Field is a spatial mapping of the potential for a force to exist (e.g., electric field, gravitational field).
...
home theory, healing, repair, trauma theory, growth theory, spirituality, rejuvination, space for authenticity and connection, protection+community as a service, theory of paradox
...
logistic regression theory, policy+procedure+process+program+project theory

agents generate/write "rules" that are generated by systems, encoded/codified by laws and realized through grammars. non adherence + challenge to these "rules" causes tension/stress/distress/discontentment leading to build up of "energy" (anger, violence, outbursts, sadness, unbounded emotion, change (in temperment), etc...) and subsequent "ripples" in "the social fabric". agents constantly attempt to balance themselves (and others) within the larger "social fabric" (the social field), and manage the "energy" (disruptions, stress, tension, etc...) that permeates throughout this field. Too much social fabric disruption (rippling) can tear agents (and agent relationships/connections) asunder, while too little social fabric challenge causes agent (and agent relationship) ossification which potentially (and inevitabilty/invariably) causes agent/agent-relationship fragility (a.k.a either agents/agent-relationships "explode into tiny pieces like a bursting balloon" (too much entropy) or "break into tiny pieces like the breaking of glass" (too much complexity)), duality is necessary for survival (identity invariance across space-time-entropy-complexity levels (level theory))




So my best guess as to why ChatGPT (and perhaps all LLMs) do the whole “not this but that thing” is that this is fundamentally (at the primitive level) how they create the sense of “boundary”. I’ll also posit that this is how humans fundamentally create boundary and distinction, using symbolic language, by saying what “some thing” is and subsequently what “some thing” is not, so I would say that this is phenomena is a feature of LLMs (and humans), not a bug, as distinction is what creates meaning, and meaning is what allows us to survive persistently (maintain an invariant sense of “self” (identity))

The "this not that hypothesis" ~ all human text (and language that produces text) on some level boils down to "this not that", but human language and expression of language has much "layered complexity" (noise) and so this fundamental (primitive) expression mechanism may not always be clear (coherent). LLMs, which consume a vast amount of human language text, are optimized to compute patterns within this text in the most "efficient" way possible (the most clear/defined/coherent-signal) and perhaps then are (indirectly) optimizing the projection of this "not this but that" mechanism into its generated output (as this mechanism likely exists in some form/at some level in all of the text that it analyzed/optimizes over), as it is "cutting through all the noise" to produce the most coherent pattern completion signal possible, given its constraints (architecture defined boundaries).
...
theory of scarcity+inclusion+exclusion+urgency+eclusiviness+anticipation/wait-listing+alluring of that which one "cannot have" but "strongl ydesires"+secrecy+secret societies ~ on the creation and implications of "scarcity" (and the illusion of scarcity as a mechanism to create the "gradient of progress/change") - change requires a  "gradient of action" which is created by boundary/limitation (a.k.a. scarcity) where scarcity is "manufacturable" and "sellable" + legitemacy theory / the theory of legitamate/illegitamate systems/grammars/structures/flows + the theory of validity/truth
...
the theory of scarcity+limitation+boundary+constraint+value+gradient of transform / entropic+complex-ic degrees of freedom / secrecy+secret societies  vs transparency
